...
public class PongApp extends GameApplication {
    ...
    private void resetBall() {
        ball.setPosition(getAppWidth() / 2F - BALL_SIZE / 2F, getAppHeight() / 2F - BALL_SIZE / 2F);
    ...
    private Entity spawnBat(int x, int y) {
        return entityBuilder()
                .at(x, y)
    ...
    protected void initInput() {
        onKey(KeyCode.W, () -> {
            if (paddle1.getY() == 0) {
                paddle1.translateY(0);
            } else {
                paddle1.translateY(-PADDLE_SPEED);
            }
        });
        onKey(KeyCode.S, () -> {
            if (paddle1.getY() == getAppHeight() - paddle1.getHeight()) {
                paddle1.translateY(0);
            } else {
                paddle1.translateY(PADDLE_SPEED);
            }
        ...
        onKey(KeyCode.DOWN, () -> {
            if (paddle2.getY() == getAppHeight() - paddle2.getHeight()) {
                paddle2.translateY(0);
            } else {
                paddle2.translateY(PADDLE_SPEED);
            }
